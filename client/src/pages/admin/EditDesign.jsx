...
const EditDesign = () => {
    ...
    const fetchDesign = async () => {
        try {
            ...
            setExistingImage(design.image || design.images?.[0] || "");
        } catch (error) {
    ...
    if (!title && status !== "loading" && status !== "success" && status !== "error") {
        return (
    ...
    return (
        <div className="min-h-screen bg-background pt-24 pb-12">
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="mb-8">
                    <Link to="/admin/designs" className="flex items-center gap-2 text-primary hover:text-primary-light transition-colors uppercase text-sm tracking-widest mb-4">
                        <ArrowLeft className="w-4 h-4" /> Back to Catalog
                    </Link>
                    <h1 className="font-serif text-3xl md:text-4xl text-white">Edit Design</h1>
                </div>
                ...
                <div className="bg-surface border border-border rounded-sm p-8 shadow-[0_0_20px_rgba(0,0,0,0.5)]">
                    ...
                    {status === "success" && (
                        <div className="mb-6 p-4 bg-green-900/30 border border-green-500 flex items-center gap-3 text-green-200">
                            <CheckCircle className="w-5 h-5" /> Design updated successfully! Redirecting...
                        </div>
                    )}
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-textSecondary uppercase tracking-widest mb-2">Update Image (Optional)</label>
                            {existingImage && <p className="text-sm text-textSecondary mb-2">Current Image: <a href={existingImage} target="_blank" rel="noreferrer" className="text-primary hover:underline">View</a></p>}
                            <div className="border border-border border-dashed bg-background p-8 flex flex-col items-center justify-center text-center cursor-pointer hover:border-primary transition-colors">
                                ...
                                <span className="text-textSecondary text-sm mb-2">{image ? image.name : "Select new image to replace current"}</span>
